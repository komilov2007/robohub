...
import type { RegisterFormType, RegisterPayload, RegisterResponse } from '@/@type/type';
...
export const usePage = () => {
  ...
  const schema = yup.object({
    ...
    password: yup
      ...
      .matches(/[!@#$%^&*()_\-+=[\]{};':"\\|,.<>/?]/, 'password_special_required')
      .default(''),
 ...
 const registerMutation = useMutation({
  mutationFn: async (data: RegisterFormType) => {
    ...
    const res = await api.post<RegisterResponse>('account/register/', payload);
    return res.data;
  },
onSuccess: (res, variables) => {
  ...
  if (accessToken) {
    document.cookie = `access_token=${accessToken}; path=/register/success; samesite=strict`;
  }

  if (refreshToken) {
    document.cookie = `refresh_token=${refreshToken}; path=/register/success; samesite=strict`;
  }
